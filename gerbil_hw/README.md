...
The RoboClaw Hardware Interface enables integration of a BasicMicro RoboClaw motor controller into ROS 2 via `ros2_control`.

This package has only been tested with ROS 2 Humble.

This repository is a robot workspace (not the upstream standalone repo). The key ROS 2 packages here are:
- `roboclaw_hardware_interface`: ros2_control `SystemInterface` plugin for RoboClaw
- `roboclaw_serial`: header-only C++ RoboClaw packet-serial driver used by the hardware interface
...
```bash
colcon build --packages-select roboclaw_serial roboclaw_hardware_interface
source ~/ros2_ws/install/setup.bash
```

3. **(Recommended) Create a stable serial device name**

If you don’t want to hardcode `/dev/tty*`, create a udev rule that symlinks your RoboClaw USB/UART adapter.

- Find the device and its attributes: `ls -l /dev/serial/by-id/`
- Create a rule in `/etc/udev/rules.d/99-roboclaw.rules` (example):

```udev
SUBSYSTEM=="tty", ATTRS{idVendor}=="XXXX", ATTRS{idProduct}=="YYYY", SYMLINK+="roboclaw", MODE:="0660", GROUP:="dialout"
```
...
Hardware and joint parameters must be provided via your robot description (URDF / xacro).

1. **Update Robot's URDF**:

The controller manager parses hardware parameters from the `<ros2_control>` block in the URDF and passes them to this hardware interface.
...
```bash
source ~/ros2_ws/install/setup.bash
ros2 launch roboclaw_hardware_interface bringup.launch.py
```
