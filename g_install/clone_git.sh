...
if [ -n "$(find . -maxdepth 1 -type d -name "gerbil-software" -print -quit 2>/dev/null)" ]; then
    alr_installed="1"
    echo "True, all set."
    echo "Now you will need to install docker for your OS. Go to https://www.docker.com/get-started/ and get Docker. Once you finish setting it up, press 1. "
    read reader_input
